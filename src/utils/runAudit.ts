...
export async function fetchPageSpeedData(url: string): Promise<PageSpeedData> {
  ...
  const hasTitleTag = audits['document-title']?.score === 1;
...
export function buildAuditResult(
  ...
  const websiteSeo: AuditCategory = {
  ...
  const googleListing: AuditCategory = {
  ...
  const socialPresence: AuditCategory = {
  ...
  const reviewVelocity: AuditCategory = {
  ...
  const overallScore = Math.round(
    (seoScore + gbpScore + socialScore + reviewScore) / 4
  );
